...
const ApiMenu = ({
  ...
  const customModel =  useStore((state) => state.customModel);
  const setCustomModel = useStore((state) => state.setCustomModel);
  ...
  const [_customModel, _setCustomModel] = useState<string>(customModel || '');

  const handleSave = () => {
    setApiKey(_apiKey);
    setApiEndpoint(_apiEndpoint);
    setIsModalOpen(false);
    setCustomModel(_customModel);
  };
      ...
      <div className='p-6 border-b border-gray-200 dark:border-gray-600'>
        ...
        <div className='flex gap-2 items-center justify-center mt-2'>
          ...
          <input
            ...
            value={_customModel}
            onChange={(e) => {
              _setCustomModel(e.target.value);
            }}
          />
        </div>
